...
import AdditionalFieldInput from "./components/AdditionalFieldInput.vue";
...
const save = () => {
    form.put(route('admin.settings.payment-methods.update'))
}
...
<template>
    ...
    <section>
        <SettingsSection v-for="method in form.methods" :key="method.key" :title="method.name">
            ...
            <template v-for="field in method.additional_fields">
                <FormField :label="field.label">
                    <AdditionalFieldInput
                        :id="`${method.key}-${field.key}`"
                        :input="field.input"
                        v-model="field.value" />
                </FormField>
            </template>
